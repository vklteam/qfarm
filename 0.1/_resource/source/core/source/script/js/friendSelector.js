...
QZONE.friends.Selector = {
	...
	/**
	 * 默认配置
	 * @param {Number} uin 当前用户的QQ号码
	 * @param {Boolean} multiChoose 是否多选
	 * @param {String} module 模块配置,QQ好友必选,1为加载,0为不加载,依次是{'关注好友','QQ号码','最近选择'}
	 * @param {Number} row 好友出现的列数
	...
	getFriendList : function(){
		...
		sender.onError = function(){
			fs.showMsgbox("由于网络或服务器问题，操作失败，请稍后再试。",1,4000);
		};
	 ...
	 */
	 cacheListData : function(re){
		...
		if(re&&re.items&&re.items.length>0){
			...
			fs.showMsgbox("您现在还没有好友。",1,2000);
		}else{
			fs.showMsgbox(re.error.msg||"获取数据失败，请稍后再试。",1,3000);
		}
	 },
	...
	getRemarkList : function(uins){
		...
		if(QZONE.FP&&QZONE.FP.getRemarkList){
			...
			sender.onError = function(){
				this.showMsgbox("由于网络或服务器问题，操作失败，请稍后再试。",1,4000);
			};
			sender.send("_callback");
		}
	},
	...
	renderRemark :  function(re,uins){
	 ...
	 createSelectUI : function(){
		...
		$('#search-box').innerHTML = '<div class="fs_search"><input id="search-input" type="text" value="查找好友" class="fs_hint" /><a id="search-button" href="javascript:;" onclick="QZONE.friends.Selector.cancelSearch();return false;" class="fs_op">查找</a></div>';
		...
		if(fsd.module=='000'){
			$('#title').innerHTML = '<div class="fs_tit">我的好友</div>';
		}
	 	...
	 	QZFL.event.addEvent($("search-input"),'keyup',function(){
	 		...
	 		if(keywords.length>0){
	 			if(fs._lastKey!=keywords){
	 				...
	 				$('#search-list').innerHTML ='<img src="view/qf_default/images/loading.gif" style="margin:10px 0px 0px 90px;" alt="正在加载"/>';
	 ...
	 createResultUI : function(){
	 	...
	 	results.innerHTML='<div class="gb_friend_selector_tit"><div class="fs_tit">选中的好友(<em id="result-count">0</em>/'+fsd.limit+')</div><div class="fs_tit_op"><a href="javascript:;" onclick="QZONE.friends.Selector.clearUp();this.blur();return false;" class="fs_empty">清空</a></div></div>'
	 	+'<div class="gb_friend_selector_cont"><div class="gb_friend_selector_list fs_list_selected"><ul class="fs_clear" id="result-list"></ul>'
	 	+'<p id="overflow" class="fs_list_overflow fs_hit none"><span class="fs_ico_warn"></span>超出可选好友上限，最多可选择'+fsd.limit+'位好友。</p></div></div>';
	 },
	 ...
	 sendEntryRight : function(uin){
	 	...
	 	$('#result-li-'+uin).innerHTML='<a href="javascript:;" onclick="QZONE.friends.Selector.delEntry('+uin+');return false;" class="fs_trash">删除</a><a href="javascript:;" onclick="this.blur();return false;" class="fs_m">'+remark+'</a>';
	  ...
	  */
	  selectGroup :  function(gpid){
	  	...
	  	var title = select ? '删除整组':'添加整组';
	  	if(!fsd.multiChoose){
	  		fs.showMsgbox('您只能选择一个好友哦',1,2000);
	  		return;
	  	}
	 ...
	 getCareList :  function(){
		...
		sender.onSuccess = function(re){
			if(re&&re.items){
				...
				if(careInGroup){
					fs.groupData.push({"gpid":9999,"gpname":"关注好友"});
					...
					if(care_list.length>0){
						...
						for(var i=0;i<care_list.length;i++){						 	
							...
							care_li.innerHTML='<span class="fs_checked" id="checked-'+care_list[i].uin+'" style="display:none;">已选中</span><a href="javascript:;" onclick="QZONE.friends.Selector.selectEntry('+care_list[i].uin+');return false;" class="fs_m">'+care_list[i]['name']+'</a>';
							fs.addHover('care-li-'+care_list[i].uin);
							fs.friendHash[care_list[i].uin+''] = care_list[i];//将好友的信息加进friend hash里
						}
						fs.careIsShow = true;
					}else{
						$('#care-list').innerHTML="<div style='margin:10px 0px 0px 10px;'>暂无关注友人</div>";
					}
				}
		...
		sender.onError = function(){
			fs.showMsgbox("由于网络或服务器问题，操作失败，请稍后再试。",1,2000);
			if(careInGroup){
				fs.buildGroupData();
			}
		};
	...
	showResultList : function(re){
		...
		if(re.length==0){
			$('#search-list').innerHTML = '<p class="fs_error_msg" id="search-hint">无好友搜索结果...</p>';
		}else{
	...
	renderResultList :  function(re){
		...
		for(var i=0;i<re.length;i++){
			...
			uin=o.uin;
		 	...
		 	$('#search-li-'+uin).innerHTML='<span id="search-checked-'+uin+'" class="fs_checked" style="display:'+(fs.isOk(uin)?"":"none")+';">已选中</span><a href="javascript:;" onclick="QZONE.friends.Selector.selectEntry('+uin+');return false;" class="fs_m" id="search-nick-'+uin+'">'+remark+'</a>';
