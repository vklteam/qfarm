...
foreach($beastsinfo['info_nc'] as $key => $value) {
	if($slotid == $value['id'] && $index == $key && (int)$value['isfarm'] == $isfarm){
		if($value['fid']<1 && $owner==0 ){
			...
			if($type=='Gun'){//用工具
				...
				$addmoral = 1;
				$subblood = round(($beastsinfo['sys_nc'][$slotid]['returntime'] - $_QFG['timestamp'])/360) - $leftblood;
				$beastsinfo['sys_nc'][$slotid]['returntime'] -= $leftblood*360;
				$beastsinfo['sys_nc'][$slotid]['curblood'] -= $leftblood;
				...
				if(mt_rand(1,5)<2){
					$result = 3;
					$addmoral = 0;
					$subblood = round(($beastsinfo['sys_nc'][$slotid]['returntime'] - $_QFG['timestamp'])/360);
					$leftblood = 0;
				} else {
					$result = 1;
	...
	$beastsinfo['info_nc'][$key]['attack'][] = array("fid"=>$_QFG['uid'],"flag"=>$flag,"id"=>$weapon,"isqz"=>true,"issucc"=>true);
