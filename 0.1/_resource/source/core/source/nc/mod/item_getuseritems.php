...
foreach((array)$dog['dogs'] as $key => $value) {
	if( $value['status'] == 1 && array_key_exists($key + 40000, $toolstype)) {
		...
		$get_exp = 0;
		$validTime = $value['dogUnWorkTime'];
		if($value['dogUnWorkTime'] == 1) $validTime=0;
		$current_str[] = '{"created":0,"itemId":' . (80000 + $key) . ',"itemType":8,"validTime":'.$validTime.',"status":' . $value['status'] . ',"id":' . (80000 + $key) . ',"itemName":"' . $get_itemName . '","price":' . $get_price . ',"exp":' . $get_exp . '}';
	}
	if( $value['dogUnWorkTime'] == 1 || $value['dogUnWorkTime']>$_QFG['timestamp'] && array_key_exists($key + 40000, $toolstype)) {
		$validTime = $value['dogUnWorkTime'];
		if($value['dogUnWorkTime'] == 1) $validTime=0;
		$get_itemName = $toolstype[40000 + $key]['tName'];
		$get_price = $toolstype[40000 + $key]['price'];
		$get_exp = 0;
		$decorative_str[] = '{"created":0,"itemId":' . (80000 + $key) . ',"itemType":8,"validTime":'.$validTime.',"status":' . $value['status'] . ',"id":' . (80000 + $key) . ',"itemName":"' . $get_itemName . '","price":' . $get_price . ',"exp":' . $get_exp . '}';
	} else {
		$up = 1;
		unset($dog['dogs'][$key]);
	}
