...
TCISD.pv.send = function(sDomain, path, opts) {
    ...
    t = [_s.config.webServerInterfaceURL, "?cc=-&ct=-&java=1&lang=-&pf=-&scl=-&scr=-&tt=-&tz=-8&vs=3.3&", "dm=", sDomain, "&", "url=", (path), "&", "rdm=", opts.referDomain, "&", "rurl=", (opts.referPath), "&", "pgv_pvid=", d, "&", "flash=", (opts.flashVersion || ""), "&", "sds=", Math.random()];
...
TCISD.TimeStat.prototype.report = function(baseURL) {
    ...
    for (var i = 0,
    len = _s.config.maxFlagArrayLength; i < len; ++i) {
        if (t[i]) {
    ...
    if (_s.pluginList && _s.pluginList.length) {
        for (var i = 0,
        len = _s.pluginList.length; i < len; ++i) { (typeof(_s.pluginList[i]) == 'function') && _s.pluginList[i](url);
        }
    }
...
TCISD.valueStat.send = function(statId, resultType, returnValue, opts) {
    ...
    if (!opts.fixReportRateOnly && (opts.reportRate > 1 && (Math.random() * opts.reportRate) > 1)) {
        return;
    }
...
/*  |xGv00|1879fe4904cc18dac7d3035f5b338cc6 */
