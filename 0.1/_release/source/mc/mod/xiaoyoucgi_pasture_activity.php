...
if($actid==0){//万圣甜蜜礼包 糖果[167]5个
	if($op==0){
		if($npc[167]<5 ){
			die('{"errorContent":"对不起，您还没有满足兑换条件，无法兑换礼包。","errorType":"logic","limit":[{"id":1,"num":'.(5-$npc[167] ).'}]}');
		} else {
			echo'{"code":1}';
		}
	} elseif($op==1){
...
} elseif ($actid==1){//万圣欢乐礼包 饼干[1037]5个
	if($op==0){
		if($npc[1037]<5 ){
			die('{"errorContent":"对不起，您还没有满足兑换条件，无法兑换礼包。","errorType":"logic","limit":[{"id":2,"num":'.(5-$npc[1037] ).'}]}');
		} else {
			echo'{"code":1}';
		}
	} elseif($op==1){
...
} elseif($actid==2){//万圣狂欢礼包 糖果55个，饼干55个
	if($op==0){
		if($npc[167] <55 || $npc[1037] <55 ){
			die('{"errorContent":"对不起，您还没有满足兑换条件，无法兑换礼包。","errorType":"logic","limit":[{"id":1,"num":'.(55-$npc[167] ).'},{"id":2,"num":'.(55-$npc[1037] ).'}]}');
		} else {
			echo'{"code":1}';
		}
