...
if($_GET['submit'] == 1 && $type != "additem") {
	...
	$nc_black = intval($_REQUEST['nc_black']);
	...
	if( $nc_black < 0 || $nc_black > 24 || $nc_redland < $nc_black ) {
		die('1|&|修改失败,红土地数目应大于1且小于24.');
	} 
	//修改farmlandstatus数据 -- START --
	$list = $_QFG['db']->fetchOne("
		SELECT N.status,N.black,U.vip FROM app_qqfarm_nc N 
...
$decorative['item3' ] = intval($_REQUEST['item3']);
	...
	$black['number'] = $nc_black;
	//修改farmlandstatus数据 -- OVER --
	$_QFG['db']->query("UPDATE app_qqfarm_user set yb=" . $yb . ",money=" . $money . ", vip='".qf_encode($vip)."' where uid=" . $id);
	$_QFG['db']->query("UPDATE app_qqfarm_nc set exp=" . $nc_exp . ",reclaim=" . $nc_reclaim . ",redland=" . $nc_redland . ", black='" . qf_encode($black) . "', status='" . qf_encode(array_values($status)) . "' where uid=" . $id);
