...
if(intval($_REQUEST['ownerId']) == $_QFG['uid']) {
	...
	if($tId==8 && (int)$cropstype[$status[$place]['a']]['isFlower'] ==0 ){
		die('{"code":0,"direction":"只能对鲜花使用！","farmlandIndex":' . $place. ',"poptype":1}');
	}

	/*if($status[$place]['bitmap'] == 2){
		$zuowutime = $_QFG['timestamp'] - $status[$place]['q'] - (0.2 * $cropstype[$a]['growthCycle']);
	}else {
	...
	if($zuowutime >= $cropstype[$a]['growthCycle']) {
		...
		} elseif( $status[$place]['bitmap']==2 && (int)$cropstype[$a]['isRed'] != 2 ){
			$status[$place]['k'] = intval($cropstype[$a]['output']*1.2);
			$status[$place]['m'] = intval($cropstype[$a]['output']*1.2);
		} else {
