...
if($_REQUEST['type'] == "0") {
	...
	if($fruit[$id] < 1 || $fruit[$id] < $foodnum) {
		...
		//输出信息
		die('{"addExp":' . $addExp . ',"added":' . $foodnum . ',"animal":' . $newanimal . ',"direction":"成功添加<font color=\"#009900\"> <b>' . $foodnum . '</b> </font>棵牧草","money":0,"total":' . $feed[animalfood] . ',"type":0,"uId":' . $uId . ',"ecode":0,"code":1,"poptype":3}');
	}
}
elseif($_REQUEST['type'] == '1') {
	...
	if($sql1) $query = $_QFG['db']->query($sql1);
	//输出信息
	echo stripslashes('{"addExp":0,"added":0,"alert":"成功购买' . $foodnum . '棵牧草，共花费金币' . ($mc_price * $foodnum) . '，已放入您的饲料机内。","animal":' . $newanimal . ',"money":' . ($mc_price * $foodnum) . ',"total":' . $feed[animalfood] . ',"type":1,"uId":' . $_QFG['uid'] . ',"ecode":0}');
	...
	//输出信息
	echo stripslashes('{"addExp":' . $addExp . ',"added":0,"animal":' . $newanimal . ',"alert":"成功购买' . $foodnum . '棵牧草，共花费金币' . ($mc_price * $foodnum) . '，已放入您的饲料机内。","money":' . ($mc_price * $foodnum) . ',"total":' . $feed[animalfood] . ',"type":2,"uId":' . $uId . ',"ecode":0,"code":1,"poptype":3}');
}
