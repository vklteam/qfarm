	...
	if(height > 0) {
		if(!loadPlayer.status) {
			...
			swfobject.embedSWF(
				...
				"mp3Player", "100%", "100%",
				"9.0.124", "source/script/swfobject/expressInstall.swf",
...
function dlBox2_Show() {
	SQuery.ajax({
		url: 'mync.php?mod=friend&refresh=true',
		success: function(data) {
			...
			SQuery("dlBox2_body").innerHTML = fcode || data;
			dlBox.Show('dlBox2_main');
		},
		error: function() {
			SQuery("dlBox2_body").innerHTML = "服务器请求错误.";
			dlBox.Show('dlBox2_main');
		}
	});
}
