...
if($feed['animalfood'] <= 0) {
	die( '{"addExp":0,"animal":{"buyTime":'.$animal[$serial]["buyTime"].',"cId":'.$animal[$serial]["cId"].',"growTime":'.$animal[$serial]["growtime"].',"growTimeNext":'.$animaltime[$animal[$serial]["cId"]][3].',"hungry":1,"serial":'.($serial+1).',"status":3,"statusNext":6,"totalCome":'. $animaltype[$animal[$serial]["cId"]]["output"] .'},"errorCode":"-11000","errorContent":"动物挨饿啦，缺少牧草会停止生产，快去添加","errorType":"1011"}');
}
if($_QFG['timestamp'] - $animal[$serial]['postTime'] <= $animaltime[$animal[$serial]['cId']][3]) {
	die( '{"addExp":0,"animal":{"buyTime":'.$animal[$serial]["buyTime"].',"cId":'.$animal[$serial]["cId"].',"growTime":'.$animal[$serial]["growtime"].',"growTimeNext":'.$animaltime[$animal[$serial]["cId"]][3].',"hungry":0,"serial":'.($serial+1).',"status":3,"statusNext":6,"totalCome":'. $animaltype[$animal[$serial]["cId"]]["output"] .'},"errorCode":"-11002","errorContent":"您下手太慢啦,已被处理.","errorType":"1016"}');
}
...
if($animal[$serial]['postTime'] == 0) {
	...
	if($chk_time < $animaltime[$animal[$serial]['cId']][0] + $animaltime[$animal[$serial]['cId']][1]) {
		die( '{"addExp":0,"animal":{"buyTime":'.$animal[$serial]["buyTime"].',"cId":'.$animal[$serial]["cId"].',"growTime":'.$animal[$serial]["growtime"].',"growTimeNext":'.$animaltime[$animal[$serial]["cId"]][3].',"hungry":0,"serial":'.($serial+1).',"status":3,"statusNext":6,"totalCome":'. $animaltype[$animal[$serial]["cId"]]["output"] .'},"errorCode":"-11000","errorContent":"还没到生产的时间呢，请不要着急","errorType":"1011"}');
	}
} else {
