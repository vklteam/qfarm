...
$beasts = $_QFG['db']->result('SELECT beasts FROM app_qqfarm_user where uid=' . $uId);
...
$user_str = '{"headPicBig":"' . qf_getheadPic($uId, 'big') . '","homePage":"' . qf_getHomePage($uId) . '","money":' . $money . ',"moralexp":'.(int)$beasts['moralexp'].',"uExp":' . $list['exp'] . ',"uId":' . $uId . ',"uLevel":' . qf_toLevel($list['exp']) . ',"uName":"' . qf_getUserName($uId) . '"}';
