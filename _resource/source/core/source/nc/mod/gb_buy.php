...
if($type == 4 || $type == 1) {//买狗
...
} elseif($type == 13) {//升级红土地
	qf_getCache('upgrade', '/nc/');
	$list = $_QFG['db']->fetchOne("SELECT status,reclaim,redland,exp FROM app_qqfarm_nc where uid=" . $_QFG['uid']);
	$status = qf_decode($list['status']);
	$level = qf_toLevel($list['exp']);
	$place = $ai-130001;
	$upid = $ai-130000;
	if($vip['status']) {
		$buy_fb = ceil($upgrade[$place]["yb"]*0.8);
	} else {
		$buy_fb = $upgrade[$place]['yb'];
	}
	...
	if($level < $upgrade[$place]['level'] || $upid <= $list['redland']){
	...
	echo '{"tId":' . $upid . ',"tName":"第'.$upid.'块红土地","code":0,"msg":"购买成功。","num":1,"fb":-' . $buy_fb . ',"money":0,"type":' . $type . '}';

} elseif($type == 12) {//升级黑土地
	qf_getCache('upblack', '/nc/');
	$list = $_QFG['db']->fetchOne("SELECT status,black,redland,exp FROM app_qqfarm_nc where uid=" . $_QFG['uid']);
	$status = qf_decode($list['status']);
	...
	$upid = $ai-120000;
	if($vip['status']) {
		$buy_fb = ceil($upblack[$place]["yb"]*0.8);
	} else {
		$buy_fb = $upblack[$place]['yb'];
	}
	...
	if($level < $upblack[$place]['level'] || $upid <= $black['number'] || $status[$place]['bitmap'] != 1){
		die('{"code":0,"direction":"请不要用非法手段升级黑土地！"}');
	} else {
		$status[$place]['bitmap'] = 2;
		$black['number'] = $upid;
		$black['cd'] = $_QFG['timestamp'] + 86400;
		if($status[$place]['k']>0){
			$status[$place]['k'] = intval($status[$place]['k']*(1+0.2)); 
			$status[$place]['m'] = intval($status[$place]['m']*(1+0.2));
		}
		$_QFG['db']->query("UPDATE app_qqfarm_user set yb=" . ($fb-$buy_fb) . " where uid=" . $_QFG['uid']);
		$_QFG['db']->query("UPDATE app_qqfarm_nc set status='".json_encode(array_values($status))."' , black='".json_encode($black)."'  where uid=".$_QFG['uid']);
	}

	echo '{"tId":' . $upid . ',"tName":"第'.$upid.'块黑土地","code":0,"msg":"购买成功。","num":1,"fb":-' . $buy_fb . ',"money":0,"type":' . $type . '}';

}elseif($type == 46) {//黄金土地

	$gold_tools = $_QFG['db']->result("SELECT black FROM app_qqfarm_nc where uid=" . $_QFG['uid']);
	$gold_tools = qf_decode($gold_tools);
	...
	$_QFG['db']->query("UPDATE app_qqfarm_nc set black='" . qf_encode($gold_tools) . "' where uid=" . $_QFG['uid']);
	$_QFG['db']->query("UPDATE app_qqfarm_user set yb=" . ($fb-$buy_fb) . " where uid=" . $_QFG['uid']);

	

	echo '{"tId":' . $tid . ',"tName":"' . $tName . '","code":0,"msg":"购买成功。","num":' . $num . ',"fb":-' . $buy_fb . ',"money":0,"type":46}';
