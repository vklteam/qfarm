...
foreach($place as $id) {
	...
	$p = (array)$status[$id]['p'];
	if($f > 0) {
		$cnt_you = array_key_exists($_QFG['uid'], $status[$id]['weed']) ? $status[$id]['weed'][$_QFG['uid']] : 0;//得到你在此地种草的数目
		if($f <= $cnt_you) {
	...
	} else die('{"code":0,"direction":"这块地不需要除草啦！","farmlandIndex":' . $id . ',"poptype":1,"weed":' . $f . '}');
	foreach($p as $pk => $pv) {
		if($pv == 2) {
			unset($p[$pk]);
			break;
