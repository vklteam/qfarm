...
function qf_getHomePage($uid = 0) {
	global $_QFG;
	$url = preg_replace('/source\/plugin.*$/i', '' ,$_SERVER['PHP_SELF']);
	return $url . "home.php?mod=space&uid=" . ($uid > 0 ? $uid : $_QFG['uid']);
}
...
function qf_getFriends($uid = 0) {
	...
	$uid = $uid > 0 ? $uid : $_QFG['uid'];
	$friends = array();
	$flist = $_QFG['db']->fetchAll("SELECT fuid FROM pap_tbpre_home_friend WHERE uid='{$uid}'");
	foreach($flist as $f) {
		$friends[] = $f['fuid'];
	}
...
function feed_add($icon, $title_template = '', $title_data = array(), $body_template = '', $body_data = array(), $body_general = '', $images = array(), $image_links = array(), $target_ids = '', $friend = '', $appid = '', $returnid = 0) {
	...
	//去重
	$oldfeed = $_QFG['db']->fetchOne("SELECT feedid FROM pap_tbpre_home_feed WHERE uid='$feedarr[uid]' AND hash_data='$feedarr[hash_data]' LIMIT 0,1");
	if($oldfeed) {
		updatetable('home_feed', $feedarr, array('feedid' => $oldfeed['feedid']));
		return 0;
	}
	//插入
	if($returnid) {
		return inserttable('home_feed', $feedarr, $returnid);
	} else {
